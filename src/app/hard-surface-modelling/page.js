...
const setupThreeJSViewer = (mountElement, modelPath, setIsLoadingCallback, modelScale = 0.1) => {
  ...
  loader.load(
    ...
    (error) => {
      ...
      setIsLoadingCallback(false);
      mountElement.textContent = 'Failed to load 3D model. Please check the file path and ensure it\'s a valid FBX.';
...
export default function HardSurfaceModellingPage() {
  ...
  useEffect(() => {
    ...
    return () => {
      if (model1MountRef.current) {
        observer.unobserve(model1MountRef.current);
      }
    };
  }, []);

  // Effect for the second model (lazy loaded)
  useEffect(() => {
    const observer = new IntersectionObserver((entries) => {
    ...
    return () => {
      if (model2MountRef.current) {
        observer.unobserve(model2MountRef.current);
      }
    };
  }, []);

  // Effect for the third model (lazy loaded)
  useEffect(() => {
    const observer = new IntersectionObserver((entries) => {
    ...
    return () => {
      if (model3MountRef.current) {
        observer.unobserve(model3MountRef.current);
      }
      observer.disconnect(); 
    };
  }, []);
